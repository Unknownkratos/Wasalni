<Page name="home">
  ...
  {#if $isConnected==true}
    <p>User Connected</p>
  {/if}
  {#if f7.store.getters.countUsers.value > 1}
    <p>User Connected</p>
  {/if}
...
<script>
  ...
  import { isConnected } from './main/connected.svelte';

  f7.store.state.users.push({id: '45'});
  

  // assignemt to new value - REACTIVE
  f7.store.state.utilisateurs = [{idd: '44'}];

  let countUsers = store.getters.countUsers.value;
